...
content_frame = tk.Tk()
content_frame.title("Sorting Algorithm Time Calculator")  # Set the window title

# Create the canvas and scrollbar for the entire GUI
canvas = tk.Canvas(content_frame, width=1200, height=800)
...
scrollbar.grid(row=0, column=2, rowspan=50, sticky="ns")  # Scrollbar in the second column, fills vertically

# Configure the grid to expand the canvas
content_frame.grid_rowconfigure(0, weight=1)  # Make row 0 expandable
content_frame.grid_columnconfigure(0, weight=1)  # Make column 0 (canvas) expandable

# Function to update the scroll region after content changes
def configure_scrollregion(event):
    canvas.configure(scrollregion=canvas.bbox("all"))

# Bind the configure event to the content_frame
content_frame.bind("<Configure>", configure_scrollregion)

# Mouse wheel scrolling function (works across platforms)
def on_mousewheel(event):
    if event.state & 0x0001:  # Check if Shift key is pressed (horizontal scrolling)
...
# Create a frame to hold the plot (inside content_frame)
plot_frame = tk.Frame(content_frame)
plot_frame.grid(row=14, column=0, sticky="nsew")
plot_frame.grid_forget()  # Initially hide the plot frame
...
def plot_times(times, canvas_frame):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))  # Ensure the figure is large enough for scrolling
    ...
    canvas_plot = FigureCanvasTkAgg(fig, master=plot_frame)  # Attach the plot to the plot_frame
    canvas_plot.draw()  # Render the plot

    # Add the plot to the Tkinter window
    plot_widget = canvas_plot.get_tk_widget()
    plot_widget.grid(row=0, column=0, sticky="nsew")  # Positioning the widget in plot_frame

    # Update the canvas to ensure proper layout
    plot_widget.update_idletasks()

    # # Now apply the scrollregion to the plot widget
    # plot_widget.config(scrollregion=plot_widget.bbox("all"))
    #
    # # Create a scrollbar and link it to the canvas
    # scrollbar = tk.Scrollbar(canvas_frame, orient="vertical", command=plot_widget.yview)
    # scrollbar.grid(row=0, column=1, sticky="ns")  # Adjust position to the right of the plot
    #
    # # Link scrollbar to the plot widget
    # plot_widget.configure(yscrollcommand=scrollbar.set)
    # scrollbar.config(command=plot_widget.yview)
    #
    # # Ensure the scroll region is updated after placing the plot
    # plot_widget.config(scrollregion=plot_widget.bbox("all"))
...
choice_label = tk.Label(content_frame,text="Choose a sorting algorithm:\n1. MergeSort\n2. HeapSort\n3. QuickSort\n4. Insertion Sort\n5. Radix Sort\n6. All")
choice_entry = tk.Entry(content_frame)
sort_button = tk.Button(content_frame, text="Next", command=on_sort)


# Widgets for array input options (initial setup)
input_choice_label = tk.Label(content_frame, text="Do you want to (1) input your own array or (2) generate a random array?")
input_choice_entry = tk.Entry(content_frame)
input_choice_button = tk.Button(content_frame, text="Proceed", command=on_input_choice, state="active")
...
input_array_label = tk.Label(content_frame, text="Enter your array (space-separated):")
input_array_entry = tk.Entry(content_frame)

# Widgets for generating a random array (initial setup)
array_size_label = tk.Label(content_frame, text="Enter the array size:")
array_size_entry = tk.Entry(content_frame)

array_range_label = tk.Label(content_frame, text="Enter the minimum and maximum values (separated by space):")
array_range_entry = tk.Entry(content_frame)
...
result_text_widget = tk.Text(content_frame, height=10, width=60)
...
scrollbar2 = tk.Scrollbar(content_frame, orient="vertical", command=result_text_widget.yview)
scrollbar2.grid(row=13, column=2, sticky="ns", padx=5, pady=10)  # Placed on the right side

# Grid first scrollbar with padding
scrollbar.grid(row=0, column=2, rowspan=20, sticky="ns", padx=(10, 0))  # Adds 10 pixels of padding to the right side

# Configure the Text widget to update the scrollbar when it scrolls
result_text_widget.config(yscrollcommand=scrollbar2.set)

# Optionally, allow the grid to expand as needed for the text widget
content_frame.grid_rowconfigure(13, weight=1)
content_frame.grid_columnconfigure(0, weight=1)  # Allow the result_text_widget column to expand
content_frame.grid_columnconfigure(1, weight=0)  # Keep the middle column fixed
content_frame.grid_columnconfigure(2, weight=0)  # Keep the scrollbar column fixed
...
content_frame.grid_rowconfigure(13, weight=1)
content_frame.grid_rowconfigure(14, weight=3)  # Adjust weight for the plot frame to allow more space
content_frame.grid_columnconfigure(0, weight=1)
